...
def save_episodes(directory, episodes, ep_id):
  directory = pathlib.Path(directory).expanduser()
  directory.mkdir(parents=True, exist_ok=True)
  timestamp = datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
  if len(episodes) > 1:
    print("notice start saving at {}th episodes".format(len(episodes)))
  for episode in episodes:
    identifier = str(ep_id)
...
def get_id(path):
  return int(str(path).split('/')[-1].split('-')[1])
...
def load_episodes(directory, rescan, buffer_size, length=None, balance=False, seed=0):
  directory = pathlib.Path(directory).expanduser()
  ...
  while True:
    # time_log = time.time()
    path_list = []
    for d2m_file_name in directory.glob('*.npz'):
      path_list.append(d2m_file_name)
    path_list.sort(key=epi_num)
    for filename in path_list:
      if filename not in name_collection:
        try:
          ...
          continue
        name_collection.append(filename)
        cache_id = get_id(filename)
        print("buffer load id {} to position: {}".format(cache_id, str(cache_id % buffer_size)))
        cache[str(cache_id % buffer_size)] = episode
    # print("line_1:{}", time.time() - time_log)
    # time_log = time.time()
    keys = list(cache.keys())
    # print("sample index:", keys)
    for index in random.choice(len(keys), rescan):
      # print("line_2:{}", time.time() - time_log)
      # time_log = time.time()
      ...
      # print("line_3:{}", time.time() - time_log)
      # time_log = time.time()
      yield episode
